...
class MongoDbMigrate:
    ...
    def collect_table_names(self):
        ...
        self.table_list = {
            migrate_table.migrationTable.SourceTable.name
        ...
        for table in self.table_list:
            if table not in tables:
                raise TableDoesNotExists(
                    f"Exception raised: Given {table} table does not exists, please check your tables"
                )
    ...
    def check_concatenate_tables_existence(self):
        ...
        tables = {
            concatenateTables.destinationColumn.options.concatenateTable
            for migrated_columns in self.config.migrationTables
            for concatenateTables in migrated_columns.migrationTable.MigrationColumns
            if concatenateTables.destinationColumn.options.concatenateTable is not None
        }
    ...
    def extract_data(self):
        ...
        for table in self.config.migrationTables:
            ...
            for migrating_columns in table.migrationTable.MigrationColumns:
                ...
                if COUNT == columns:
                    self.destination_table_operations(
                        current_table, destination_collection
                    )
                    current_table = {}
    ...
    def destination_table_operations(self, table_info, destination_collection):
        ...
        try:
            ...
            for key, value in table_info.items():
                ...
                if (
                    value.get("destinationColumn")
                    .get("options")
                    .get("concatenateTable")
                ):
                    ...
                    document.update({value.get("destinationColumn").get("name"): ""})

                    temp.update(
                        {
                            value.get("destinationColumn")
                            .get("name"): value.get("destinationColumn")
                            .get("options")
                            .get("type_cast")
                        }
                    )
                    ...
                    old_columns.update(
                        {
                            value.get("destinationColumn")
                            .get("name"): value.get("sourceColumn")
                            .get("name")
                        }
                    )
