...
class PostgresqlFullMigration:
    ...
    def clone_database(self):
        for schema in self.schemas:
            ...
            source_db_metadata = (
                self.source_db_operations.get_reflected_metadata_of_schema(
                    schema=schema
                )
            )
            self.destination_db_operations.create_all_tables_with_metadata(
                source_db_metadata
            )
    ...
    def drop_all_tables_constraints_in_destination_db(self):
        for schema in self.schemas:
            ...
            for table in tables:
                fks = self.destination_db_operations.get_foreign_keys_constraints(
                    table.name, schema=schema
                )
                if not fks:
                    logger.info(
                        f"Foreign Key not found for table {table} in schema {schema}"
                    )
                    continue
                for fk in fks:
                    logger.info(
                        f'Dropping foreign key constraint. schema: {schema} table: {table.name} fk: {fk["name"]}'
                    )
                    self.destination_db_operations.drop_constraint(
                        fk["name"], table.name, "foreignkey", schema=schema
                    )
        ...
        for schema in self.schemas:
            ...
            for table in tables:
                ...
                for data in yield_data:
                    self.destination_db_operations.insert_data(
                        table_name=table.fullname, data=data._asdict(), schema=schema
                    )

    def __set_foreign_key_constraints_in_destination_table(self, table, schema):
        fks = self.source_db_operations.get_foreign_keys_constraints(
            table_name=table, schema=schema
        )
        if not fks:
            logger.info(
                f"Foreign key constraint not found for table {table} in source database. Skipping..."
            )
            return

        for fk in fks:
            logger.info(f"Setting foreign key constraint in table {table.name}")
            self.destination_db_operations.set_foreign_key_constraint(
                source_table=table.name, fk_options=fk
            )
    ...
    def set_foreign_key_constraints_in_destination_db(self):
        for schema in self.schemas:
            ...
            tables = self.source_db_operations.get_all_tables(schema=schema)
            for table in tables:
                self.__set_foreign_key_constraints_in_destination_table(
                    table.name, schema
                )

    def migrate_all_stored_functions_and_procedures_from_source_db_to_destination_db(
        self,
    ):
    ...
    def migrate_foreign_data_wrappers(self):
        ...
        for fdw in fdws:
            ...
            # fmt: off
            fdwowner = (self.source_db_operations.get_user_name_by_object_id(fdwowner_oid)) # noqa F841
            fwdhandler = object_ids.get(fdwhandler_oid)
            fdwvalidator = object_ids.get(fdwvalidator_oid)
            logger.info(
                f"Creating foreign data wrapper fdw={fdwname} handler={fwdhandler} validator={fdwvalidator}"
            )
            self.destination_db_operations.create_foreign_data_wrapper(
                foreign_data_wrapper=fdwname, handler=fwdhandler, validator=fdwvalidator
            )
    ...
    def migrate_foreign_servers(self):
        for (
            ...
            port,
            dbname,
        ) in self.source_db_operations.get_fwd_servers_from_pg_database():
            logger.info(
                f"Creating foreign. server: {server_name} dbname: {dbname} host: {host} port: {port}"
            )
    ...
    def migrate_user_mappings(self):
        for (
            ...
            user_name,
            options,
        ) in self.source_db_operations.get_user_mappings():
            logger.info(
                f"Creating user mapping server: {server_name} user: {user_name} options: {options}"
            )
            self.destination_db_operations.create_user_mapping(
                server_name, user_name, options
            )
    ...
    def __generata_table_structure(self, foreign_table):
        ...
        for columns in foreign_table:
            foreign_table_name, schema_name, column_name, column_type, ord_number = (
                columns
            )
    ...
    def migrate_foreign_tables(self):
        ...
        for foreign_table in foreign_tables_list:
            table_data = (
                self.source_db_operations.get_foreign_table_column_informations(
                    foreign_table_name=foreign_table
                )
            )
            foreign_table_structure = self.__generata_table_structure(table_data)
            server_name = self.source_db_operations.get_server_name_of_foreign_table(
