...
class DBOperations:
    ...
    def get_object_ids_for_function_names(self):
        ...
        for f in functions_and_procedures:
            schema_name, function_name, kind, language, defintion, function_args, return_type = f

            statement = "SELECT * FROM pg_get_object_address('function', '{%s}', '{%s}');" % (function_name, function_args)
            try:
                object_addresses = self.execute_stmt(statement, log_stmt_exception=False).fetchall()
    ...
    def create_foreign_data_wrapper(self, foreign_data_wrapper, handler=None, validator=None):
